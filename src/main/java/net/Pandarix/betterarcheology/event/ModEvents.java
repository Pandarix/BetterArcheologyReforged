    ...
    public static void addCustomTrades(VillagerTradesEvent event)
    {
        if (event.getType() == ModVillagers.ARCHEOLOGIST.get())
        {
            ...
            trades.get(5).add((trader, rand) -> new VillagerTrades.TreasureMapForEmeralds(
                    13,
                    TagKey.create(Registries.STRUCTURE, BetterArcheology.createResource( "on_catacombs_explorer_map")),
                    "filled_map.catacombs",
                    MapDecorationTypes.WOODLAND_MANSION, 12, 5).getOffer(trader, rand));

            trades.get(5).add((trader, rand) -> new VillagerTrades.TreasureMapForEmeralds(
                    13,
                    TagKey.create(Registries.STRUCTURE, BetterArcheology.createResource( "on_light_explorer_map")),
                    "filled_map.light",
                    MapDecorationTypes.WOODLAND_MANSION, 12, 5).getOffer(trader, rand));
        }
    }
